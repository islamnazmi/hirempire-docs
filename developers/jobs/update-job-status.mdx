...
The new status for the job. Accepted input values: `Pending` `Active` `Paused` `Hired` `Closed`. Values are normalized to lowercase before storage. `Pending` maps to `draft`. `Hired` is accepted as a **deprecated alias** for `Closed`. The response always returns the canonical lowercase value: one of `draft` `active` `paused` `closed`.
</ParamField>
